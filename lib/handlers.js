...
handlers.fb_leadgen_webhook = (data, callback) => {
  ...
  for (let queryKey in queryStringObject) {
    if (queryKey == 'hub.challenge') {
      queryStringObject.hub_challenge = queryStringObject[queryKey]
    }
    else if (queryKey == 'hub.verify_token') {
      queryStringObject.hub_verify_token = queryStringObject[queryKey]
    }
  }
  if (queryStringObject.hub_verify_token == 'falconagencyrocking123') {
    return queryStringObject.hub_challenge;
  }
  callback(null, queryStringObject.hub_challenge);
}
