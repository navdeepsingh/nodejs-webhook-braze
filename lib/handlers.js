...
handlers._user.post = (data, callback) => {
  ...
  if (payload.default_address !== undefined) {
    ...
    payload.city = payload.default_address.city;
    payload.province = payload.default_address.province;
...
handlers._fulfillment.post = (data, callback) => {
  ...
  if (discountCodes !== undefined && discountCodes.length > 0) {
    const discountCodesObject = discountCodes[0];
    postData.events = [{
      "external_id": customerEmail,
      "name": "Discount Code Used",
      "time": payload.updated_at,
      "properties": discountCodesObject
    }]
  }
